...
const { Moves } = loadResource(POKEMON_SHOWDOWN_RESOURCE, "moves");
const { MovesText } = loadResource(POKEMON_SHOWDOWN_RESOURCE, "text", "moves");
const { LAST_GEN, movesByGen } = loadResource(LIBS, "util");

const createDiscriminate = ({
  name,
  category,
  description,
  power,
  ...
  JSON.stringify({ name, category, description, power, pp, accuracy, type });
...
const findInheritedMovesGenProperty = (gen, moveName, property) => {
  ...
  if (property === "basePower") property = "power";
...
const determineCategory = (gen, type, initialCategory) => {
  if (initialCategory === "Status") return initialCategory;
  ...
  if (gen < 4) return beforeGen4[type];
      ...
      [key]: {
        ...
        description:
          value.isNonstandard === "Past"
            ? `Not available in gen ${LAST_GEN}`
            : MovesText[key].desc || MovesText[key].shortDesc,
...
const movesCollection = Object.entries(lastGenMoves).reduce(
  (accumulator, [key, value]) => {
...
for (let gen = LAST_GEN - 1; gen > 0; gen--) {
  Object.entries(MovesText).forEach(([key, value]) => {
    if (
      lastGenMoves[key] /** to prevent accessing to NonStandardMoves */ &&
      movesByGen[
        key
      ] /** moves like 10000000voltthunderbolt are not reported to be learnt */ &&
      ...
      const moveGen = {
        ...
        category: determineCategory(
          ...
          lastGenMoves[key].category
        ),
        description:
          Object.keys(multipleDescriptionGen).length > 0 &&
          multipleDescriptionGen["gen" + gen]
            ? MovesText[key][multipleDescriptionGen["gen" + gen]].desc ||
              MovesText[key][multipleDescriptionGen["gen" + gen]].shortDesc
            : MovesText[key].desc || MovesText[key].shortDesc,
        shortDescription:
          Object.keys(multipleDescriptionGen).length > 0 &&
          multipleDescriptionGen["gen" + gen]
            ? MovesText[key][multipleDescriptionGen["gen" + gen]].shortDesc
            : MovesText[key].shortDesc,
