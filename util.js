...
const getPokemonKeyFromName = (pokemonName) => {
  ...
  let pokedexEntry = pokedexEntries.find(
    ([key, { name }]) => name === pokemonName
  );
...
let movesByGen = Object.values(Learnsets).reduce((accumulator, value) => {
  ...
  if (learnset) {
    Object.entries(learnset).forEach(([moveName, rawGenArray]) => {
      rawGenArray = Array.from(
        new Set(rawGenArray.map((rawGen) => parseInt(rawGen[0])))
      );

      if (accumulator[moveName])
        moveObjects[moveName] = Array.from(
          new Set(accumulator[moveName].concat(rawGenArray))
        );
      else moveObjects[moveName] = rawGenArray;
    });
  }
