...
const itemsCollection = Object.entries(Items)
  .filter(
    ([key, { isNonstandard }]) =>
      ...
      !key.match(/tr\d+/g) // pokemon shouldn't hold TRs --> we remove those items
  )
...
Object.keys(itemsCollection).forEach((key) => {
   ...
   */
  const otherGens = getGenAttributes(ItemsText[key])
  ...
  } else if (Items[key]["gen"])
    //If no other gen found, it means that the item didn't change its description since the original gen (found in gen attribute of Items[key])
