...
import { insertOrUpdate } from '../../db/db';
...
describe('Testing description override after import', () => {
	...
	test('For item table', async () => {
		try {
			...
			const result = await transaction
				...
				.first();
			// The description should be what we've used for the manual update
			expect(result.description).not.toBe('Indisponible dans cette génération.');
	...
	test('For move table', async () => {
		try {
			...
			expect(result.description).toBe('Non disponible en génération 8 : UPDATE');
		} catch (error) {
			console.log(error);
		}
	});
