...
bluebird.map(learns,async (object) =>
{
    ...
    return { INSERTED: 1 };
}, { concurrency: 150 } ).then((results) => {
    console.log({ table: 'pokemon_move', INSERTED: results.reduce((sum,{ INSERTED }) => { sum += INSERTED; return sum; },0)})
})
